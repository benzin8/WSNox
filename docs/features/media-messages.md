...
| | Фото | Видео |
|---|---|---|
| Форматы | jpeg / png / webp | mp4 / quicktime / webm |
| Максимальный размер | 10 MB | 50 MB |
| Серверная обработка | Pillow: downscale до 1920 по большой стороне, JPEG q=82, отдельный thumb 320×320 | RAW upload (без транскода), client-side meta (width/height/duration) |
| Подпись | ✅ опционально | ✅ опционально |
| Reply на медиа | ✅ quote показывает «Фото» / «Видео» если подписи не было | то же |
| Lightbox | ✅ фуллскрин с blur backdrop | ✅ фуллскрин c controls |
...
| Функция | Что делает |
|---|---|
| `process_image(storage, user_id, file)` | streaming read с `FileTooLarge`, Pillow `verify()` + `exif_transpose` + `convert("RGB")`, downscale, два JPEG в S3 |
| `process_video(storage, user_id, file, client_meta_raw)` | streaming read, валидация mime, `_validate_video_meta` bound-checks `width/height/duration_ms`, raw upload |
...
POST /chats/{chat_id}/media   (multipart/form-data)
  file:         UploadFile  (image/* | video/*)
  caption:      str = ""
  reply_to_id:  int | null
  client_meta:  str  (JSON: {width, height, duration_ms}) — только для видео
...
- **ffmpeg + серверный thumbnail видео** — пока браузер делает poster из
...
`tests/test_media_pipeline.py` — 17 кейсов:
