...
export function AccountsBlock({ onAddAccount }) {
  ...
  // Clicking a row: switch if the account is signed in; if it's signed out
  // (needs_login), there's no refresh cookie to mint from — so send the user
  // to re-authenticate instead of silently doing nothing.
  const handleRowClick = (acc, isActive) => {
    if (isActive) return;
    if (acc.needs_login) handleAdd();
    else switchAccount(acc.user_id);
  };
        ...
        {accounts.map((acc) => {
          ...
          return (
            <li
              ...
              onClick={() => handleRowClick(acc, isActive)}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                {acc.needs_login && (
                  <div className="text-xs text-amber-400">Войти →</div>
                )}
              </div>
              {count > 0 && (
